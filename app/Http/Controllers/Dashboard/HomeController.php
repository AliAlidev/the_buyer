...
class HomeController extends Controller
{
    ...
    public function recover_password($token, $user_id)
    {
        $user = User::find($user_id);
        if (!$user)
            return $this->sendErrorResponse("Validation errors", "User not found!");

        $fp = DB::table('password_resets')->where('email', $user->email)->where('token', $token)->first();
        $valid_date = Carbon::parse($fp->created_at)->addHours(6);
    ...
    public function viewInvoice($order_number)
    {
        ...
        if ($invoice) {
            ...
            $customer = $invoice->invoice_type == 1 ? $invoice->Store->name : $invoice->customer->name;
